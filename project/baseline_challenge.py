...
class BaselineChallenge(FlowSpec):
    ...
    def baseline(self):
        ...
        self._name = "baseline"
        params = "Majority Class Predictor"
        ...
        self.predictions = dummy.predict (x_val)
        self.scores = dummy.predict_proba(x_val)

        self.acc = acc(y_val, self.predictions)
        self.rocauc = rocauc(y_val, self.scores[:, 1])
        print(f"Baseline performance: \n{self.acc} Accuracy\n {self.rocauc} ROC AUC")
       
        self.result = ModelResult("Baseline", params, pathspec, self.acc, self.rocauc)
